...
public class AnnuncioConverter extends AbstractConverter<Annuncio,AnnuncioDTO> {
	...
	public Annuncio toEntity(AnnuncioDTO annuncioDTO) {
		...
		if (annuncioDTO != null) {
			annuncio = new Annuncio(annuncioDTO.getId(), annuncioDTO.getContratto(),annuncioDTO.getDataPubblicazione(),annuncioDTO.getDescrizione(),sedeRepository.findById(annuncioDTO.getSede()).get(),settoreRepository.findById(annuncioDTO.getSettore()).get(), null);			
		}
		return annuncio;
	}
	...
	public AnnuncioDTO toDTO(Annuncio annuncio) {
		...
		if (annuncio != null) {
			annuncioDTO = new AnnuncioDTO(annuncio.getId(),annuncio.getDescrizione(), annuncio.getContratto(),annuncio.getDataPubblicazione(),annuncio.getSede().getId(),annuncio.getSettore().getId());			
		}
		return annuncioDTO;
	}
